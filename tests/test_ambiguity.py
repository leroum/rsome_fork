...
def test_integer_scen():
    ...
    x = model.dvar(5)
    z = model.rvar((3, 5))
    assert z.__repr__() == "0: 3x5 random variables"
    assert E(z).__repr__() == "E(0): 3x5 expectation of random variables"
    ...
    fset[4:7].exptset(
        rso.norm(E(z[0])) <= 1.5, rso.norm(E(z[1])) <= 1.0, rso.norm(E(z[2])) <= 0.5
    )
    events = fset.showevents()
